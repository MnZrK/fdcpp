...
let main argv =
    ...
    try
        ...
        async {
            let! message = readMessageAsync()
            logger.Info "got message: %s" message
            
            let dcppMsg = Dcpp.parseMessage message
            match dcppMsg with
            | Some (Dcpp.HelloMessage {lock = lock; pk = pk}) ->
                logger.Info "got lock %s" lock
                logger.Info "got pk %s" pk
            | _ -> failwith "Could not parse hello message"
